...
FacStatus Facility::Request(Token tkn, int pri)
  ...
  // Seleciona disciplina FIFO (para usar com PRI Adaptativo)
  //fq->Insert (evnt, AT_REAR);
...
void Facility::Release (int tkn_id, int classe, int max) //Mario
  {
  Boolean want_snapshot = Future::SnapshotFlag();
  ...
  Event eee;
  if (max < 0)
     eee = fq->Remove (FROM_FRONT);
  else
     eee = fq->RemoveSel (MATCHING_CLASS, classe, max);
  //
